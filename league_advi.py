...
from league_load_game_data import load_game_data
from utils.emp_bayes import calc_priors
...
def run_league_advi(df, priors, state_mapping):
    """
...
def plot_model_results(trace, state_mapping):
    ...
    plt.tight_layout()
    fig.savefig('manpower_scoring_rates.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    posterior_trace, model = run_league_advi(df_ready, priors_df, mapping)
    
    save_trace_to_netcdf(
        posterior_trace,
        "advi_traces/league_nhl_trace.nc",
        success_msg="Saved successfully to advi_traces/league_nhl_trace.nc",
    )
